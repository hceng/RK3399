...
static bool firefly_long_wrt_switch = true;
...
static unsigned long rockchip_dsi_calc_bandwidth(struct dw_mipi_dsi *dsi)
{
	...
	if (dsi->lvds_force_clk != -1){
	target_mbps = dsi->lvds_force_clk;
        printf("force target_mbps=%ld \n", target_mbps);
	}
	return target_mbps;
}
...
#define FDT_GET_INT(val, name) \
	...
	if (dsi->lvds_force_clk < 0){
		printf("Can't get <dsi,lvds-force-clk>\n");
	}
